...
class InteractiveMultiMenu {
        ...
        switch ($keyPress.Key) {
            ...
            $([ConsoleKey]::Spacebar) {
                ...
                if ($selectedItem.Readonly) {
                    $this.Errors += "$($selectedItem.Label) is marked as readonly and cannot be changed"
                } else {
            ...
            Default {
                $this.Errors += "Unkown key pressed: $_. Press 'h' for help."
            }
        }
        return $true
    }
    ...
    hidden OpenBrowser() {
        ...
        if (-not [string]::IsNullOrEmpty($selectedItem.Url)) {
            ...
            $this.Errors += "$($selectedItem.Label) doesn't have an URL"
        }
    }
    ...
    hidden ShowErrors() {
        ...
        if ($this.Errors.Count -gt 0) {
            $this.Errors | ForEach-Object {
                Write-Host "$_$bufferFill" -ForegroundColor $this.MenuErrorColor
            }
            $this.Errors = @()
        } else {
...
class ChooseMenuOption {
    [ValidateNotNullOrEmpty()][string]$Label
    [ValidateNotNullOrEmpty()][string]$Value
    [ValidateNotNullOrEmpty()][string]$Info

    ChooseMenuOption([string]$label, [string]$value, [string]$info) {
        $this.Label = $label
        $this.Value = $value
        $this.Info = $Info
    }
...
class ChooseMenu {
    ...
    [ValidateNotNullOrEmpty()][ChooseMenuOption[]]$Options
    ...
    hidden [int]$CurrentIndex = 0
    hidden [ChooseMenuOption]$SelectedOption = $null
    hidden [bool]$Help = $false
    hidden [string[]]$Errors = @()

    ChooseMenu([string]$question, [ChooseMenuOption[]]$options) {
    ...
    hidden ShowErrors() {
        if ($this.Errors.Count -gt 0) {
            Write-Host
            $this.Errors | ForEach-Object {
                Write-Host "$_" -ForegroundColor $this.ErrorColor
            }
            $this.Errors = @()
        }
    }
...
Export-ModuleMember -Function Get-InteractiveMultiMenuOption,Get-InteractiveMenuUserSelection